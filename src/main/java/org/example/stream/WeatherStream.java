...
public class WeatherStream {
    public static void main(String[] args) {
        ...
        final Map<String, String> serdeConfig = Collections.singletonMap(
                "schema.registry.url", "http://<HOST_IP>:8081"
        );

        final SpecificAvroSerde<Weather> weatherSerde = new SpecificAvroSerde<>();
        weatherSerde.configure(serdeConfig, false);

        final SpecificAvroSerde<AggregatedWeather> aggregatedSerde = new SpecificAvroSerde<>();
        aggregatedSerde.configure(serdeConfig, false);

        // Build stream topology
        KStream<String, Weather> weatherStream = builder.stream(
                "weather-topic",
                Consumed.with(Serdes.String(), weatherSerde)
        );

        KGroupedStream<String, Weather> groupedStream = weatherStream
                .groupBy(
                        (key, weather) -> weather.getLocation().toString(),
                        Grouped.with(Serdes.String(), weatherSerde)
                );

        KTable<String, AggregatedWeather> aggregatedTable = groupedStream.aggregate(
                () -> new AggregatedWeather("", 0.0, 0, 0.0),
                (key, weather, aggregate) -> {
                    aggregate.setLocation(key);
                    aggregate.setTotalTemperature(aggregate.getTotalTemperature() + weather.getTemperature());
                    aggregate.setCount(aggregate.getCount() + 1);
                    aggregate.setAverageTemperature(
                            aggregate.getTotalTemperature() / aggregate.getCount()
                    );
                    return aggregate;
                },
                Materialized.<String, AggregatedWeather, KeyValueStore<Bytes, byte[]>>as("weather-aggregates-store")
                        .withKeySerde(Serdes.String())
                        .withValueSerde(aggregatedSerde)
        );

        aggregatedTable.toStream()
                .to("aggregated-weather-topic", Produced.with(Serdes.String(), aggregatedSerde));
